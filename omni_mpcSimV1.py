...
import math
import time
from my_package_py.mpc_omni import MPC_solver
from my_package_py.omni_buffalo import Omni_model
from my_package_py.bezier_path import calc_bezier_path
...
class Simulation_MPC(Node):
     def __init__(self):
        ...
        #   self.control_sub = self.create_subscription(String, '/control', self.sub_control,10)
          ...
          self.omni = Omni_model(0.05,0.12,0.12)
          self.timer_run = self.create_timer(timer_run, self.timer_runcallback)
          #Setup MPC
          self.lowX = [-15, -15, -3.14]
          self.highX = [15, 15, 3.14]
          self.lowU = -30.0
          self.highU = 30.0
        #   Q = [1500, 1200, 1000]  # range Q the best is Qx=1500-1200 , Qy = 1500-1000 , Qyaw = 1000-800 use travel =2
          Q = [1500, 1500, 900] # Use travel 3
          R = [1, 1, 1, 1]
          self.N = 45
          self.T = 0.1
        ...
        #   self.X_end1 = [5.5, -2.6, -1.57]  # 
        #   self.X_end2 = [7.8, -2.4, 1.57]
        #   self.X_end3 = [7.8, -1.5, -1.57]  #Ball
        ...
        #   self.X_end8 = [11.15, -4.28, -1.57]
          ### #  Test in lab
        #   self.X_End = [0.0, 0.0, 0.0]
        #   self.X_end  = [3.0, 0.0, 0.0]    
        #   self.X_end1 = [3.50, -2.5, -0.0]  # 
        #   self.X_end2 = [3.5, -1.0, 1.57]
        # #   self.X_end3  = [3.0, 0.5, 0.0] 
     ...
     def subscript_goal(self,goal):
          if (goal.data == "success"):
                ...
                path4, _ = self.calc_4points_bezier_path(self.X_end2, self.X_end3, 3.0, 50)
     ...
     def callback_timer(self):
         ...
         self.solx = sol_x.full()[0, :]
         self.soly = sol_x.full()[1, :]
         self.theta = sol_x.full()[2, 0]
         ...
         for i in range (self.N):
                for_vec = self.omni.forward_kinematic(self.u1,self.u2,self.u3,self.u4,self.theta,"numpy")
                self.V = np.sqrt(for_vec[0]**2+for_vec[1]**2)
                self.travel += abs(self.V)*self.T
     ...
     def timer_runcallback(self):
         for_vec = self.omni.forward_kinematic(self.u1,self.u2,self.u3,self.u4,self.theta,"numpy")
         v = np.sqrt(for_vec[0]**2+for_vec[1]**2)
         x_next = self.current_states + self.omni.forward_kinematic(self.u1, self.u2, self.u3, self.u4, self.theta,"numpy") * self.sampling_time
         ...
         plt.scatter(self.solx, self.soly, marker="*", color="red", label="Predicted value")
