...
class Omni_model():
    ...
    def inverse_matrix(self, type=None):
        ...
        if type == "numpy":
            J_inv = (1/self.R)*np.array([[-1, 1, self.L],
                                         [-1,-1, self.L],
                                         [ 1,-1, self.L],
                                         [ 1, 1, self.L]],dtype=np.float64)
            
        elif type == "sym":
            J_inv = (1/self.R)*ca.vertcat(
                ca.horzcat(-1, 1, self.L),
                ca.horzcat(-1,-1, self.L),
                ca.horzcat( 1,-1, self.L),
                ca.horzcat( 1, 1, self.L)
            )
    ...
    def forward_kinematic(self, W1, W2, W3, W4, theta, type=None):
        ...
        if type == "numpy": 
            ...
            V = rot.T @ self.forward_matrix(type) @ ca.vertcat(ca.horzcat(W1),ca.horzcat(W2),ca.horzcat(W3),ca.horzcat(W4))
    ...
    def inverse_kinematic(self, Vx, Vy, Omega, type=None):
        ...
        if type == "numpy":
            ...
            W = self.inverse_matrix(type) @ ca.vertcat(ca.horzcat(Vx),ca.horzcat(Vy),ca.horzcat(Omega))
