...
class Simulation_MPC(Node):
     def __init__(self):
          ...
          #Setup MPC
          self.lowX = [-15.0, -8.0, -3.14]
          self.highX = [15.0, 8.0, 3.14]
          self.lowU = -40.0
          self.highU = 40.0
          Q = [750, 750, 850]
          R = [0.1, 0.1, 0.1, 0.1]
     ...
     def feedback_states(self,V_external):
     ...
     def subscript_goal(self,goal):
          if (goal.data == "success"):
                self.X_start = [0.0,0.0,0.0]
                path1, _ = self.calc_4points_bezier_path(self.X_start, self.X_end, 1.7, 60)
                path2, _ = self.calc_4points_bezier_path(self.X_end, self.X_end1, 2.5, 50)
                ...
                self.goal2 = np.vstack([path2[:, 0], path2[:, 1], np.append(np.arctan2(np.diff(path2[:, 1]), np.diff(path2[:, 0])), -1.57)])
     ...
     def callback_timer(self):
        ...
        if (self.mode_status == "manual"):
            ...
            for i in range (self.N):
                    for_vec = self.omni.forward_kinematic(self.current_controls[0],self.current_controls[1],self.current_controls[2],self.current_controls[3],theta,"numpy")
            ...
            self.get_logger().info('Velocity 4 wheel: [%f, %f, %f]' % (self.current_states[0],self.current_states[1],self.current_states[2]))
            
            # Step2: Rotary and IMU 
