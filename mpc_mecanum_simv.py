...
import math
import time
from my_package_py.MPC_controller import MPC_solver
from my_package_py.mecanum_buffalo import BuffaloModel
...
class Simulation_MPC(Node):
     ...
     def callback_timer(self):
         ...
         for i in range (self.N):
                for_vec = self.mecanum.forward_kinematic(self.u1,self.u2,self.u3,self.u4,theta,"numpy")
                ...
                if (self.target_ind + self.index) < len(self.X_ref):
                        ...
                        self.next_trajectories[1, i+1] = self.Y_ref[len(self.Y_ref)-1]
                        self.next_trajectories[2, i+1] = self.Yaw_ref[len(self.Yaw_ref)-1]
         ...
         x_next = self.current_states + self.mecanum.forward_kinematic(self.u1, self.u2, self.u3, self.u4, theta,"numpy") * self.T
